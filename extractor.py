...
st.caption("Upload nutrition labels, get JSON back — no schema, no tools, just valid JSON.")
...
def robust_json_from_text(txt: str, product_id: str) -> Dict[str, Any]:
    ...
    Returns a dict with at least product_id.
    """
    # Fast path
    try:
        data = json.loads(txt)
...
def extract_from_image_bytes_freeform(
    ...
    """
    Responses API (no tools, no response_format). We ask for a plain JSON object in text.
    ...
    if not txt and hasattr(resp, "model_dump"):
        try:
            ...
            if not txt:
                # Try walking output/content for text chunks
    ...
    return robust_json_from_text(txt or "{}", product_id)
...
if run:
    ...
    for idx, up in enumerate(uploads[:total], start=1):
        ...
        try:
            img_bytes = up.getvalue()  # use getvalue so we can read multiple times if needed
            payload = extract_from_image_bytes_freeform(client, model, img_bytes, filename, product_id)
